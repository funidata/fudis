...
Breadcrumbs Item has two mandatory properties: string `label` and Angular Router string `url`. Label is the visible label text and url is the actual url for the Angular router.

The last Breadcrumbs Item Component is always displayed as non-link and just plain text. This is achieved through CSS and doesn't require configuration from the application.
...
<ArgTypes of={BreadcrumbsItemComponent} />
