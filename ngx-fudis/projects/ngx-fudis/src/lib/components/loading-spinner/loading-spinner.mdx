import { ArgTypes, Meta, Canvas, Source } from "@storybook/addon-docs/blocks";
...
- **IMPORTANT**: Instead of using `@if (appLoading) { <fudis-loading-spinner /> }` use `<fudis-loading-spinner [visible]="appLoading"/>`. This will help screen reader users, as component exists in DOM although it is not visually visible.
