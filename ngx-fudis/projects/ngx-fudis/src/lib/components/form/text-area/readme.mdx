...
[MaxLength](/docs/utilities-validators--documentation&args=#minlength-and-maxlength-validators) validator on Text Area does not prompt any error message but disable user to input more than allowed length of characters to the input field. Additionally, when maxLength validation is used, by passing maxLength Input <code>[maxLength]="maxLength"</code> to Text Area, will activate the maximum character indicator to the bottom right corner.
