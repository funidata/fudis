...
The function receives a date and must return: - `true` → the date is allowed and remains enabled in the calendar - `false` → the date is not allowed and will be disabled in the calendar
