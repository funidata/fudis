...
Datepicker allows user to select a date either from calendar dropdown or by typing in the input.
...
In addition Fudis has internal validator for date format which prompts error message if Fudis is not satisfied with given date's format.
