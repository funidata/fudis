...
Actions Directive also needs `type` input which has to be set as '_fieldset_'.
...
Additional Header content is used for adding Notifications and other components to Field Set header section.
Additional content is positioned under the Field Set label and it is assigned through Header Directive content projection with **fudisHeader** selector.
Header Directive also needs a `type` input which has to be set as '_fieldset_'.
...
Content Directive also needs `type` input which has to be set as '_fieldset_'.
