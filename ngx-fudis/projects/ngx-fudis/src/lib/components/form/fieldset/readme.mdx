...
**Note!** Fieldset Actions have an `align` property that can be used to determine the flow direction of Fieldset header elements with Fieldset Actions.

```
<fudis-fieldset-actions [align]="'start'">
  <your-content-here />
...
Fieldset Action's `align` is used for positioning Fieldset's Actions Directive flow to `bottom`, `end` or `start`.
