...
**Note!** Fieldset's `AlignActions` property can be used in positioning Fieldset's header elements along with Fieldset Actions.
...
Fieldset `alignActions` is used for positioning Fieldset's Actions Directive flow to `bottom`, `end` or `start`.
