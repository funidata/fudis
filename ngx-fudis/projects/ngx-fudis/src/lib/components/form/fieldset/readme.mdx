...
Fudis Actions Directive also needs `type` input which has to be set as '*fieldset*'.
...
Fudis Notifications Directive also needs a `type` input which has to be set as '*fieldset*'.
...
Fudis Content Directive also needs `type` input which has to be set as '*fieldset*'.
...
If the whole Field Set is required, it can be set through `required` boolean property which will add '*(Required)*' text after the legend.
