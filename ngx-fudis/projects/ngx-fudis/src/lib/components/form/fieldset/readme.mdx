...
There are no restrictions on Field Set Actions content but it is advised to use Buttons only.
**Note!** Fieldset's `AlignActions` property can be used in positioning Field Set's header elements along with Field Set Actions.
