...
Fudis Badge can be attached to form heading by adding `badge` (variant) and `badgeText` (text content) properties.
...
Fudis Form extends Fudis Grid Api Directive which allows the modification and usage of most of the [Grid properties](/docs/directives-grid-grid--documentation).
