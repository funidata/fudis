...
Both types of error messages share the same design and are always rendered as a part Guidance Component.
...
The mandatory properties for Error Message are `message` and `visible`. Also, the **NgIf** directive is required so that the Error Message can be passed to Error Summary Component. For the properties, the message parameter can be either passed as a string or as an observable string.
