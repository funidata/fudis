...
Popover can be included with properties from [Popover Directive](/docs/directives-popover--documentation). It also requires an additional `popoverTriggerLabel` for the button that triggers the popover. This label is only visible for the screenreaders. The popover button contains only an icon, so it is required to inform the user what it is for, for example _"Additional information"_.
