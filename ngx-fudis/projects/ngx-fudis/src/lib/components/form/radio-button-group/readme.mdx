...
Necessary properties for radio-button-group are `label`, `control` and `options`. Radio-button-group has to have **at least** two radio options and should **not be used** with only one option available.
