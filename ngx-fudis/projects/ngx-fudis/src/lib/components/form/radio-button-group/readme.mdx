...
Necessary properties for radio-button-group are `title`, `control` and `options`. Radio-button-group has to have **at least** two radio options and should **not be used** with only one option available. Otherwise it will throw an error.
