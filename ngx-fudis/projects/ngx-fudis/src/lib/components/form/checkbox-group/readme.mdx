...
To display visible 'required' text with the fieldset group, set `required` input as true to Checkbox Group and include validator `FudisGroupValidators.atLeastOneRequired()` in the validators array of `formGroup`.
...
If checkbox-group fieldset has minimum and/or maximum options to be selected, form field include `FudisGroupValidators.min` and/or `FudisGroupValidators.max` in the group validators.
