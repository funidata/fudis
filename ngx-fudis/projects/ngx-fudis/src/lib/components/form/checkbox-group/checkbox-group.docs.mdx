...
Property `control` is Angular FormControl typed object. When it is provided, it will be added to the internally created Form Group of the Checkbox Group. If `controlName` property is provided, it will be used as this control's key in the Form Group. Otherwise internally generated id will be used.
...
myOptionsWithControls = [
  // Note! Provided controlName property is optional this way.
...
To use validators for the whole Form Group, please provide [`formGroup` property](with-formgroup-property-in-the-parent-checkbox-group) to it.
...
myFormGroup = new FormGroup<FudisCheckboxGroupFormGroup<object>>(
  ...
  [FudisGroupValidators.atLeastOneRequired('Select at least one checkbox!')]
);
