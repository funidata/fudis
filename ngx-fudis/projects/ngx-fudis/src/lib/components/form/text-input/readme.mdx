...
If text-input is a **required** form field, include `FudisValidators.required()` in the form control validator with message parameter either as a string or as an observable string. E.g. `FudisValidators.required('This is required field')`.

#### With minLength and maxLength validators

Text-input can have min and max length validators. 

For min length, include `FudisValidators.minLength()` in the form control validator.
...
Include `FudisValidators.min()` and/or `FudisValidators.max()`in the form control validators. 
