...
Body text variants are responsive and on mobile devices, if screen size is less than `breakpoint('sm')"` (this is referencing to the mixin, hence different spelling), `lg` variants will scale down to `md`.
