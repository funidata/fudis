...
There are six header options available from **h1** to **h6** and seven size options from **xxl** to **xxs**. All of the heading levels have default sizes, where heading level **h1** is equivalent to size **xxl**, heading level **h2** is equivalent to size **xl** etc. However, heading sizes are responsive and on mobile devices where screen size is less than [breakpoint('sm')](/docs/foundations-breakpoints--documentation#breakpoint-mixins) all of the headings are converted to a smaller heading sizes.
...
<ArgTypes of={HeadingComponent} exclude={headingExclude} />
