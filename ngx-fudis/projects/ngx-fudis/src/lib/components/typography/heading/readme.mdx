...
All of the heading levels have default sizes, where heading level **h1** is equivalent to size **xxl**. Heading sizes are responsive and on mobile devices where a screen size is less than
...
Note! When a heading size is set, heading level does not have an effect to a size, i.e they are only essential for screen readers and other assistive technologies. 
