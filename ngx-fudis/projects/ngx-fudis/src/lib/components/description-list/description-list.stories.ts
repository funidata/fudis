...
const lonelyDataItem = [
  { key: 'Vastuuopettajan sähköposti', value: 'olli@ope.com', subHeading: 'Olli Opettaja' },
];
...
const DescriptionListDataLoopTemplate: StoryFn<DescriptionListComponent> = (
  args: DescriptionListComponent,
) => ({
  props: args,
  template: html`<fudis-heading [level]="2" size="md">
      Description List Regular With Data Looping</fudis-heading
    >
    <fudis-description-list
      [data]="data"
      [marginBottom]="'md'"
...
const DescriptionListWithSubComponentsTemplate: StoryFn<DescriptionListComponent> = (
  ...
  props: args,
  template: html`<fudis-heading [level]="2" size="md"
      >Description list built with sub components</fudis-heading
    >
