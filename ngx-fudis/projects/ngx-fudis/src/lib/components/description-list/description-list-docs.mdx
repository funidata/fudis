...
<fudis-dl [tag]="'p'">
  <fudis-dl-item>
    <fudis-dt [textContent]="'Single Item Key'"/>
    <fudis-dd [textContent]="'And value for that'"/>
  </fudis-dl-item>
</fudis-dl>`}
/>
...
It is possible to incluce e.g. Button component or other interactive elements inside Description List Item Details (`<fudis-dd>`). This following example displays, how Button is used inside the Description List.
