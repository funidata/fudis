...
[Tooltip](/docs/directives-tooltip--documentation) can be added to Description List Item Term (`<fudis-dt>`) element by using `tooltip`, `tooltipPosition` and `tooltipToggle` properties. While it works on both Description List variants, it is designed to be used only with **regular** variant.
