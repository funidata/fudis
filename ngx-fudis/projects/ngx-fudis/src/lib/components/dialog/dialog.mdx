...
import * as DialogFormStory from "./dialog-with-form.stories.ts";
import * as DialogGridStory from "./dialog-with-grid.stories.ts";
import * as DialogNestedStory from "./dialog-nested-example.stories.ts";
...
Pay attention to accessibility when designing dialog with read-only content.
...
>**If dialog consists ONLY text**: Set `focusToContent` input value as true. This will ensure that scrollable content is accessible to keyboard and screenreader users.
```
<fudis-dialog>
...
<Canvas of={ DialogFormStory.ExampleWithForm } />

<Canvas of={ DialogGridStory.ExampleWithGrid } />

<Canvas of={ DialogNestedStory.ExampleWithNestedDialogs } />
