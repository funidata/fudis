...
`dialogService.open()` can be called with optional `config` parameter (in addition to component or tempalte reference) which is extension from `MatDialogConfig`. It can be used to alter some dialog configs but most importantly pass `data` to Dialog's child component.
...
Optionally, form submit button can be used with `fudisFormSubmit` directive which handles Error Summary visibility and reloading the errors, see instruction from [Form Actions documentation](/docs/directives-form-actions--documentation). 
				...
				<fudis-dialog-content>
					<fudis-form>
						...
						<ng-template fudisActions type="form">
							<fudis-button (handleClick)="closeDialogWithForm()" [label]="'Submit'" [type]="'submit'"></fudis-button>
							<fudis-button fudisDialogClose [label]="'Cancel'"></fudis-button>
						</ng-template>
					</fudis-form>
				</fudis-dialog-content>
