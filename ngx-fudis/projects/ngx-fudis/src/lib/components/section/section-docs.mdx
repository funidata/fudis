...
#### Header Actions and Additional Header Content

Buttons are added through `fudisActions` directive and additional header content is added through `fudisHeader` directive. 
Header directive is mainly used for adding Notifications and other components to Section's header. In order to work they both need `type` input which has to be set as '_section_'.
Actions and Header directives have fixed positions and cannot be altered. 
