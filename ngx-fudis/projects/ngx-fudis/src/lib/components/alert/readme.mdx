...
CSS `position` of Alert Group can be set using `position` attribute. By default it is `fixed` but can be as well `static` or `absolute` currently.
...
Alerts without links are readout to screen readers, as they have html attribute of `role='alert'`. Focus does not change even if a new alert appears.
