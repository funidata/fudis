...
It has been discussed with designers, that using alerts with links should be avoided, as it can cause confusion for keyboard users. But if a link is needed both `routerLinkUrl` and `linkTitle` must be provided.
...
All alerts can be dismissed by calling
...
CSS `position` of Alert Group can be set using `position` attribute. By default it is `static` but can be as well `fixed` or `absolute` currently.

If Fudis Dialog is open, Fudis automatically has embedded Alert Group inside every Dialog, so that new and existing alerts are visible and available for keyboard users when Dialog is open.

## Accessibility and keyboard behavior

### Alerts without a link

Alerts without links are readout to screen readers, as their html `role='alert'`. Focus does not change even if a new alert appears.
...
When user closes an alert, focus will move automatically to the last alert in the list. If there are no alerts left, Alert Group tries to focus back there in the UI the focus was before focusing on the alert.

### With Fudis Dialog open

Fudis Dialog has a focus trap, that keyboard user should be not able to navigate outside dialog until it is closed again. If there are alerts visible, those are included as focusable elements inside Dialog's focus trap. If user closes all the alerts when dialog is open, focus returns to Dialog's close button.

### Other mentionable accessibility details

- Focusable elements inside alert has visible focus state.
- Alert without a link is readout to user using `role='alert'` attribute.
