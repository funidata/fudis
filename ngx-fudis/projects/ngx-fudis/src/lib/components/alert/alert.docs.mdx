...
import { AlertComponent } from "./alert/alert.component.ts";
...
If a link is needed insinde an alert, both `routerLinkUrl` and `linkTitle` must be provided.
...
Focus will move to the newly added alert's link, so that keyboard users can easily react to it if needed.
