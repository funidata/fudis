...
Reason why Expandable Content is not used with similar syntax as other content projection, is to enable lazy loading of its contents. Other content slots should be visible right from the start, but closed Expandables content is initialized only after it is opened for the first time.
