...
const ValidatorsTemplate: StoryFn = (args) => ({
	props: {
		...
		minLengthTextInput: {
			control: new FormControl('', FudisValidators.minLength(10, 'Input should be at least 10 characters long')),
		},
		maxLengthTextInput: {
			control: new FormControl('', FudisValidators.maxLength(20, 'Input should not be less than 20 characters')),
		},
		minNumberInput: {
		...
		patternTextInput: {
			control: new FormControl(null, FudisValidators.pattern(/[\d]/, 'Text should include at least one digit')),
		},
	...
	template: html`
		<fudis-grid [columns]="2" [marginBottom]="'xl'">
			...
			<fudis-text-input [label]="'Email validator'" [control]="emailTextInput.control" [type]="'email'" />
		...
		<fudis-grid [columns]="2" [marginTop]="'xl'">
			...
			<fudis-checkbox-group [title]="'Min Group Validator'" [formGroup]="checkboxGroupMin.formGroup">
				<fudis-checkbox
					*ngFor="let option of checkboxGroupMin.options"
					[controlName]="option.controlName"
					[label]="option.label" />
			</fudis-checkbox-group>
			<fudis-checkbox-group [title]="'Max Group Validator'" [formGroup]="checkboxGroupMax.formGroup">
				<fudis-checkbox
