...
When maxLength validation is used, [Text Input](/docs/components-form-text-input--documentation) and [Text Area](/docs/components-form-text-area--documentation) component's disable the user from inputing more than allowed length of characters and have additional maxLength indicator.
When using [Dropdown](/docs/components-form-dropdown--documentation) with maxLength validator, error message is prompted when the desired maximum amount of options are selected.
