...
OneRequired validator can be used with Checkbox Group and Input With Language Options, cases where at least one option needs to be selected. When using this validator, the input has to be set as required. Error message parameter is given as a string or as an observable string
