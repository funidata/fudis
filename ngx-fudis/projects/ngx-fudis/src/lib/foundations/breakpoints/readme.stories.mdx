...
The mixins are using min-width, so when you are using the mixin **breakpoint('md')**, your styles will be applied from the medium breakpoint and up.
...
The mixins are using max-width, so when you are using the mixin **breakpoint-max('md')**, your styles will be applied if viewport is small than M size.
...
| $breakpoint-lg  | m            | 62em  | 992px            | 992px-1199px     |
| $breakpoint-md  | s            | 48em  | 768px            | 768px-991px      |
| $breakpoint-sm  | xs           | 36em  | 576px            | 576px-767px      |
