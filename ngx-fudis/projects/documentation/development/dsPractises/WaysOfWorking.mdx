...
To use foundations in component's scss file, import Core with SCSS's `@use` feature and use it in similar fashion like in example below.

```
@use '@funidata/fudis-core' as core;

.fudis-my-new-component{
	@include core.color('white')
	@include core.bg-color('primary')

	&__inner-svg-icon{
		fill: core.$fudis-color-primary-light;
	}
}
