...
See [Getting Started](/docs/documentation-development-getting-started--documentation#testing) for a quick reference of VR testing commands.
...
If there are test failures, the container will not exit but rather serve the resulting test report at [http://localhost:9323](http://localhost:9323). You can also run `npm run test:vr-report` to see the latest report.
