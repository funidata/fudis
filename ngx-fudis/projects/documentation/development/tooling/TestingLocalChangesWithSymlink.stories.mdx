...
2. Run build command `npm run build:ngx-fudis` in `fudis/ngx-fudis` folder
3. Navigate to dist project  `fudis/ngx-fudis/dist/ngx-fudis`
4. Run `yarn link`
5. Running should result in success message: `success Registered "@funidata/ngx-fudis"`
6. Switch to application side
7. Add `"preserveSymlinks": true` to application's `angular.json`
...
12. Now you are using your local package
