...
.fudis-my-new-component{
	@include colorMixins.fudis-color('white')
	@include colorMixins.fudis-bg-color('primary')
...
But if ngMaterial based Fudis component requires additional tweaking in styles, create a file with rest of the component files. E. g. to `components/form/datepicker` a file `ngMaterial-theme.scss`. Also import this file to root `style.scss` file to make it available to application and ngMaterial.
