...
test("form dynamic", async ({ page }) => {
  await page.goto(
    "/iframe.html?args=&id=components-form-form--dynamic-example&viewMode=story",
  );
  ...
  await page.getByTestId("fudis-button-3").click(); /* remove max length validator from text input */
  await page.getByTestId("fudis-button-5").click(); /* remove email pattern validator */
  await page.getByTestId("fudis-button-7").click(); /* remove min length validator from email input */
